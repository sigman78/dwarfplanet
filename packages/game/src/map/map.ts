import { Biome, LAND_PASSABLE_BIOMES, FISH_PASSABLE_BIOMES } from './tiles'

export class GameMap {
  readonly width: number
  readonly height: number
  readonly biomes: Uint8Array
  private readonly spatialHash: Map<number, Set<number>> = new Map()
  ...
  isPassable(x: number, y: number, isLand: boolean): boolean {
    ...
    return isLand ? LAND_PASSABLE_BIOMES.has(biome) : FISH_PASSABLE_BIOMES.has(biome)
  }
  ...
  addEntity(id: number, x: number, y: number): void {
    const key = this.cellKey(x, y)
    if (!this.spatialHash.has(key)) this.spatialHash.set(key, new Set())
    this.spatialHash.get(key)!.add(id)
  }

  removeEntity(id: number, x: number, y: number): void {
    this.spatialHash.get(this.cellKey(x, y))?.delete(id)
  }

  moveEntity(id: number, fromX: number, fromY: number, toX: number, toY: number): void {
    this.removeEntity(id, fromX, fromY)
    this.addEntity(id, toX, toY)
  }

  getEntitiesInRadius(cx: number, cy: number, radius: number): Set<number> {
